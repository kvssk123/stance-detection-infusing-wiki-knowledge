...
if __name__ == '__main__':
    ...
    topic = ['face_masks', 'fauci', 'stay_at_home_orders', 'school_closures','hcq'][2]
    batch_size = 16
    epochs = 100
    patience = 10
    lr = 2e-5
    ...
    command = f"python3 -u src_xlm_r/train.py "               f"--data={data} "               f"--topic={topic} "               f"--model={model} "               f"--wiki_model={wiki_model} "               f"--n_layers_freeze={n_layers_freeze} "               f"--n_layers_freeze_wiki={n_layers_freeze_wiki} "               f"--batch_size={batch_size} "               f"--epochs={epochs} "               f"--patience={patience} "               f"--lr={lr} "               f"--l2_reg={l2_reg} "               f"--gpu={gpu}"               # f" > {file_name}"

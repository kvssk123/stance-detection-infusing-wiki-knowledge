...
class COVIDTweetStance(Dataset):
    def __init__(self, phase, topic, model='bert-base', wiki_model=''):
        ...
        if model == 'bert-base':
            ...
            tokenizer = AutoTokenizer.from_pretrained('sagteam/covid-twitter-xlm-roberta-large')
        ...
        if wiki_model:
            ...
            if wiki_model == model:
                tweets_targets = [f'text: {x} target: {y}' for x, y in zip(tweets, targets)]
                encodings = tokenizer(tweets_targets, [wiki_summary] * df.shape[0], padding=True, truncation=True, max_length=128)
                encodings_wiki = {'input_ids': [[0]] * df.shape[0], 'attention_mask': [[0]] * df.shape[0]}
            else:
                encodings = tokenizer(tweets, targets, padding=True, truncation=True)
                encodings_wiki = tokenizer_wiki([wiki_summary] * df.shape[0], padding=True, truncation=True,max_length=128)

        else:
            encodings = tokenizer(tweets, targets, padding=True, truncation=True,max_length=128)
            encodings_wiki = {'input_ids': [[0]] * df.shape[0], 'attention_mask': [[0]] * df.shape[0]}

        # encodings for the texts and tweets
        input_ids = torch.tensor(encodings['input_ids'], dtype=torch.long)
        attention_mask = torch.tensor(encodings['attention_mask'], dtype=torch.long)
        token_type_ids = torch.zeros(df.shape[0],dtype=torch.long)
    ...
    def __init__(self, phase, model='bert-base', wiki_model=''):
        ...
        if wiki_model:
            ...
            if wiki_model == model:
                ...
                tokenizer_wiki = AutoTokenizer.from_pretrained('xlm-roberta-large')
